...
func (c *TemporalWorkerDeploymentCreateVersionCommand) run(cctx *CommandContext, args []string) error {
	...
	if c.AwsLambdaInvoke != "" {
		...
		if err := validateAWSLambdaProviderDetails(providerDetails); err != nil {
			return err
		}
		dc := converter.GetDefaultDataConverter()
		detailsPayload, err := dc.ToPayload(&providerDetails)
		if err != nil {
			return fmt.Errorf("encoding provider details: %w", err)
		}
