...
func (v *ScheduleConfigurationOptions) buildFlags(cctx *CommandContext, f *pflag.FlagSet) {
	...
	f.StringArrayVar(&v.ScheduleMemo, "schedule-memo", nil, "Set a schedule memo using `KEY=VALUE` pairs. Keys must be identifiers, and values must be JSON values. For example: 'YourKey={\"your\": \"value\"}'. Can be passed multiple times.")
}
...
func NewTemporalWorkflowResetCommand(cctx *CommandContext, parent *TemporalWorkflowCommand) *TemporalWorkflowResetCommand {
	...
	s.Command.Flags().StringArrayVar(&s.ReapplyExclude, "reapply-exclude", nil, "Exclude these event types from re-application. Accepted values: All, Signal, Update.")
...
func NewTemporalWorkflowUpdateCommand(cctx *CommandContext, parent *TemporalWorkflowCommand) *TemporalWorkflowUpdateCommand {
	...
	s.UpdateTargetingOptions.buildFlags(cctx, s.Command.PersistentFlags())
	return &s
}
...
func NewTemporalWorkflowUpdateResultCommand(cctx *CommandContext, parent *TemporalWorkflowUpdateCommand) *TemporalWorkflowUpdateResultCommand {
	...
	s.Command.Flags().StringVar(&s.UpdateId, "update-id", "", "Update ID. Required.")
	_ = cobra.MarkFlagRequired(s.Command.Flags(), "update-id")
